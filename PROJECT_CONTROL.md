...
| 3 | Luna/X monitoring proof | **Blocked on production intake remediation** | `X_TEST_CANDIDATE_LIMIT=1 node pipeline/test-real-x-pick.js --no-discord` checked one recent `@CappersUSA` post through `gpt-5.6-luna`; outcome was `SOURCE_EXTRACTED`. The 10:00/10:15 production audit proved 12-way X intake is fast, but 853 distinct posts were deferred by the two-call cap, only four Luna runs occurred, and zero approval cards were created. | Implement deterministic/global candidate prioritization and a bounded deferred queue, test it without publication, then deliberately deploy and prove a qualifying pick reaches private approval in the same pass. See `docs/X_MONITOR_LATENCY_AUDIT_2026-09-13.md`. |
...
| 6 | Cost containment | **Contained but operationally blocked** | Luna is active; durable X cursor/dedupe uses `/var/data`; hard pre-call limits are 50 OpenAI requests/day, 500/month, $1 estimated/day, and $15 estimated/month; monitored-source publishing is disabled. The first two production cycles respected the two-call limit. | The current policy generated 994 deferral events for 853 distinct posts and cannot drain during the operating window. Fix candidate selection and deferred persistence before raising the per-run cap; then begin at six while retaining daily/monthly hard stops. |
...
| 2026-09-13 | Complete authenticated repeat-offer acceptance on the isolated legacy fixture and close the outbound Discord attribution gap. After backfilling only the pre-guard test customer marker, a fresh Discord-authenticated portal review showed no second retention offer. Staging Worker `4fe6301a-307c-446b-87ee-7a772c38bbfb` is healthy; its entitlement probe wrote a member-attributed Discord role PUT with `membership_entitlement_sync` and HTTP `204`. Full suite 177/177. | Zakai Martin | Staging passed; active repeat-test subscription awaits separately confirmed cancellation/clock advancement; production untouched |
