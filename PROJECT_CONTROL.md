...
- Fresh test clock `clock_1UFAPBE6p9BmPii34X93rbeb` now owns customer `cus_VFfml7z9hBMSMK` and active monthly subscription `sub_1UFATvE6p9BmPii3SBESa2O0`. Webhook `evt_1UFATxE6p9BmPii35o31amyx` is `PROCESSED/ROLE_GRANTED`; the Discord role PUT returned HTTP `204`. The staging customer mapping was reassigned only after the old subscription was terminal, and the reassignment was written as `STAGING_TEST_IDENTITY_REASSIGNED`.
...
- Stripe test coupon `VJxmdXFz` (`KBH 75% Retention — One Invoice`) is 75% off once and is configured in Customer Portal configuration `bpc_1UF9gPE6p9BmPii3pI6NPPAn`. Applying it to the fresh simulated subscription and advancing the test clock are prepared but intentionally await the required action-time financial confirmation. No real payment, production write, or production Discord mutation occurred.
...
- The owner selected a one-time cancellation-retention offer: 75% off the next `$32.99` monthly invoice (approximately `$8.25`), then automatic return to `$32.99/month` until canceled. Customers must be able to decline the offer and continue cancellation without obstruction. The exact test coupon and portal offer are configured; the discounted invoice, return-to-full-price invoice, decline path, and repeat-attempt behavior still require the prepared test-clock run.
...
- Checkout Worker `kobes-betting-hub-checkout-staging` is deployed at `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev`. Source commit `e31e234966aae3b721bc75dc0c2853632a9d7110` is deployed as version `531ee711-60a1-4429-9147-6595ed13eb5a` and includes Stripe Basil compatibility plus race-safe durable refund/dispute entitlement blocking and failed-payment auditing. Health returned `200`; all six expected encrypted secret names remain present; cron is disabled; the isolated Discord guild/active-role bindings are present; `STRIPE_GLOBAL_PAYOUTS_KEY` is intentionally absent.
...
| 2 | Membership lifecycle | **Partial; terminal grant/removal verified in staging** | Production membership controls remain untouched. Staging migration `009` is applied; exact source commit `e31e234966aae3b721bc75dc0c2853632a9d7110` is deployed as Checkout Worker `531ee711-60a1-4429-9147-6595ed13eb5a` and is healthy. Original test subscription `sub_1UF8Y4E6p9BmPii37J9VhpeS` reached `canceled`; webhook `evt_1UFAQCE6p9BmPii3yw1gVYX1` is `PROCESSED/ROLE_REMOVED`, and the Discord DELETE returned `204`. Fresh clock `clock_1UFAPBE6p9BmPii34X93rbeb` created active subscription `sub_1UFATvE6p9BmPii3SBESa2O0`; webhook `evt_1UFATxE6p9BmPii35o31amyx` is `PROCESSED/ROLE_GRANTED`, and the Discord PUT returned `204`. Refund/dispute durable blocks and failed-payment handling pass automated tests; the Stripe staging destination listens to all eight events. | Apply the staged retention coupon and advance the test clock after the required action-time financial confirmation; prove the discounted invoice, return to full price, decline/repeat behavior, terminal cancellation, Supabase state, and role removal. Then run isolated external partial/full-refund, dispute creation/resolution, failed-payment/recovery, and audited restoration fixtures plus the referral lifecycle. |
| 3 | Luna/X monitoring proof | **Complete locally and for the authorized extraction smoke test** | `X_TEST_CANDIDATE_LIMIT=1 node pipeline/test-real-x-pick.js --no-discord` checked one recent `@CappersUSA` post through `gpt-5.6-luna`; outcome was `SOURCE_EXTRACTED`. A fully mocked non-public test covers extraction → ESPN event/roster gate → identical private/public copy → canonical log → final ESPN grade/unit math → recap → X copy → Trends generation. The full local suite passes 162/162. | The next normal window must prove bounded production behavior in the durable ledger. After deliberate deployment, capture controlled real Discord, ESPN, Gmail, Trends, and X delivery evidence. Automatic grading remains limited to supported MLB/NFL/NCAAF markets; unsupported picks require `/grade-pick` so they do not hold the recap in `PENDING`. |
| 4 | Durable API trail | **Partial / deployment acceptance pending** | Supabase migrations `001`–`006` are live; explicit Node and Checkout Worker calls write endpoint/outcome/latency/hash/workflow metadata; eight production ESPN events were observed immediately after cutover. Publisher Worker `33bdf3f7-f0aa-4d8b-b133-9b0ec2212365` audits all outbound X calls. Commit `2b1fc85` additionally prepares authenticated Apps Script queue-call audit rows in append-only D1 and Discord SDK no-response failure rows in Supabase without retaining credentials, payloads, identifiers, or error text; the current full suite passes 162/162. | Deploy the Worker and Render changes, update the authorized Apps Script project, capture one normal Apps Script D1 row and one controlled Discord no-response Supabase row, then reconcile zero unclassified provider traffic for a controlled window. |
...
| 7 | Free Pick mismatch | **Production text path complete; image path prepared locally** | A dedicated least-privilege credential is encrypted in Cloudflare and Render. Canonical pick `20260912-148-X` returned `201`, the current API and public page rendered it, and today's five-check production smoke passed. Local image work now carries an approved `/publish-pick` attachment into website/X publication, accepts only non-empty JPEG/PNG/WebP up to 5 MB, preserves exact bytes/type, prevents duplicate text-plus-image X posts, and reports partial delivery. The current 162-test suite covers the combined path. | Deliberately deploy, then use one controlled approved image to verify the Discord card, website image/media bytes, exactly one X post, and audit rows. The monitored-source approval UI still cannot add an image; the usable image workflow is private `/preview-pick` followed by manual `/publish-pick` to `#daily-free-play`. |
...
| 9 | Legal/support operations | **Blocked on remaining owner/legal decisions** | Draft Terms, Privacy, support pages, and issue playbooks exist. The owner selected cancellation at period end, all sales final except legally/card-network-required or written exceptions, and a one-time 75%-off next-month retention offer before return to `$32.99/month`. Test coupon `VJxmdXFz` and the portal offer are configured; the draft Terms remain unpublished and unapproved. | Supply the legal business identity/address, jurisdiction, support/privacy email, failed-payment grace rule, referral-program authority/termination language, tax reporting process, retention policy, backup operator, and legal approval; then finish the test-clock coupon proof and publish timestamped policies. |
