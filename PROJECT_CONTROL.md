...
**Last verified:** 2026-09-13 16:24 MST
...
- **Passed:** the Apps Script source is now version-controlled with four no-send/credential/trigger invariants. Three additional tests prove exact image-byte multipart publication, text-only fallback, and fail-closed credential behavior; the complete release suite passes `184/184`.
...
| 2026-09-13 | Bring the formerly Google-only daily email implementation under repository control and add four invariants covering zero-pick silence, no empty X queue call, no embedded credentials, and trigger-disabled recap polling. Add three image-backed Free Pick transport/fallback/fail-closed tests. | Zakai Martin | Complete; release suite later expanded to `184/184` |
