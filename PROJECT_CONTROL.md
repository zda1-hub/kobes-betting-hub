...
**Last verified:** 2026-09-13 20:03 MST
...
- **Private Trends transport activated:** one dedicated credential is aligned across the Publisher Worker, Render, and Apps Script without entering the repository or this control file. Render polls every five minutes. The first poll correctly failed closed because the private approval channel was absent from `ALLOWED_CHANNEL_IDS`; the production allowlist was repaired, the replacement deployment succeeded, Kobe Bot logged in, and startup no longer reports the allowlist error. Google authorization is complete, the read-only Trends connection passed, and `queueKobeTrendEmails` is installed every five minutes with the backlog-exclusion timestamp in force.
- **Recap delivery remains fail-closed:** its Apps Script read-only connection test returned `401`, proving the stored Google recap credential does not match the active production credential. The recap trigger was deliberately not installed. The working Render credential has been located without displaying or storing its value; owner confirmation is required immediately before copying it into Google Apps Script.
...
| Trends inbox | Gmail Apps Script + D1 + Render | `cloudflare/kobe-trends-inbox.gs`, publisher Worker, bot | D1 queue + approval packet | Credential aligned; Render poll and five-minute Google trigger live; one fresh private-card acceptance remains |
| Recap email | Render + D1 + Gmail Apps Script | `bot/index.js`, `cloudflare/kobe-daily-picks-email.gs` | CSV grades + D1 notification status | Render path active; daily zero-pick email suppressed; read-only Google test fails closed with `401`; five-minute delivery trigger remains off pending credential alignment and approved-pick proof |
