...
**Last verified:** 2026-09-12 21:38 MST
...
- Checkout Worker `kobes-betting-hub-checkout-staging` is deployed at `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev`, version `397e7ee3-e79c-4a18-9c32-393eae2a59b2`. Its health check returns `200`; its cron is disabled; Stripe and Discord identifiers remain fail-closed placeholders until test-only resources are attached; `STRIPE_GLOBAL_PAYOUTS_KEY` is intentionally absent.
...
- The remaining staging blockers are account-authenticated creation of Stripe test products/prices/webhook/portal configuration and a Discord test application/guild/role. No test checkout, charge, subscription, cancellation, Discord role mutation, payout, or production write has occurred.
...
- [ ] Finish the staging environment. Separate Supabase, encrypted server credential, Checkout Worker, and static-site Worker are deployed and verified; Stripe test objects and Discord test application/guild/role remain behind their account login gates.
