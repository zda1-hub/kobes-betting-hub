...
- Checkout Worker `kobes-betting-hub-checkout-staging` is deployed at `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev`, version `397e7ee3-e79c-4a18-9c32-393eae2a59b2`. Its health check returns `200`; its cron is disabled; unresolved Discord guild/role values remain fail-closed; `STRIPE_GLOBAL_PAYOUTS_KEY` is intentionally absent.
...
- Stripe test products already present in the intended `Kobesbettinghub` test account are now pinned in staging configuration: monthly product `prod_V8mOKsNfRQSQbk` / price `price_1U8UpxE6p9BmPii3GoBjAXII` ($32.99/month) and starter product `prod_V8mOyRBS66iOwo` / price `price_1U8UpLE6p9BmPii3ErYC4FPr` ($10 one-time). A least-privilege restricted key was created but its value surfaced in browser accessibility output; it is not trusted or installed and must be revoked after a replacement is securely captured. The unrelated `Kobe Referral Sandbox` account is out of scope.
- Discord staging application `Kobe Betting Hub Staging` (`1548556799513333770`) exists and its isolated callback `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev/discord/callback` is saved. An independent `DISCORD_OAUTH_STATE_SECRET` is encrypted in Cloudflare staging. Client-secret and bot-token generation are pending owner MFA; test guild and member role remain uncreated/unrecorded. No test checkout, charge, subscription, cancellation, Discord role mutation, payout, or production write has occurred.
...
| 2 | Membership lifecycle | **Partial; isolated staging foundation deployed** | Production membership controls remain live. Separate staging Supabase `ctqmksvfqrysomvckqrm`, Checkout Worker `397e7ee3-e79c-4a18-9c32-393eae2a59b2`, and static-site Worker `1ce02b72-1f87-460e-a6b5-1187408af831` are deployed; staging schema/RLS/empty-state checks passed; cron and payouts are disabled; browser configuration fails closed across environments. Stripe test prices are identified and Discord app `1548556799513333770` plus its staging callback exist. Full local suite passes 147/147. | Securely replace and install the exposed Stripe staging key, create the correct-account webhook, configure Customer Portal cancellation, finish Discord MFA/client secret/bot token/test guild/member role, and deploy the resulting staging bindings. Then run one dedicated test identity through Checkout → link → portal → cancel → role removal and retain Stripe event IDs, Discord user/role evidence, Supabase rows, and timestamps. Separately run the referral lifecycle in Stripe test mode, including the seven-day eligibility transition and a test payout. |
...
- [ ] Finish the staging environment. Separate Supabase, encrypted server credential, Checkout Worker, and static-site Worker are deployed and verified; Stripe test prices and Discord application/callback are recorded; replacement Stripe credential, correct-account webhook/portal, Discord MFA credentials, test guild, and role remain.
