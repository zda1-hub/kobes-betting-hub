...
**Last verified:** 2026-09-13 18:20 MST
...
- **Membership import corrected:** production reconciliation now discovers Stripe subscriptions rather than only rows already known to Supabase. It imported both observed live subscriptions and carries their Stripe-stored Discord IDs into the normalized customer rows. The remaining access failure is isolated to Discord role hierarchy: the active managed role `Kobe's Betting Hub` has `Manage Roles` but sits below configured member role `VIP`. The unrelated `Kobe Bot` integration role was cleaned up by disabling its mistakenly added `Manage Roles` permission. Discord rejected an audited API attempt to raise the active bot's own managed role, so the server owner must perform the one drag in Discord's role UI.
...
| 8 | Membership reconciliation | **Partial — hierarchy acceptance waiting** | Checkout Worker `bbdf7a3e-1d2c-4ebe-a908-35416b3c0f51` runs Stripe → Supabase → Discord reconciliation daily at `15 16 * * *` (09:15 Arizona) and records outcomes/failures. The private read-only GitHub Actions alarm is active and correctly reports the two unresolved paid-role grants without identifiers. The active bot role has `Manage Roles`; the unused integration role was cleaned up. | Server owner drags `Kobe's Betting Hub` above `VIP`; then rerun reconciliation, require two role grants, and require the alarm to return clean. |
