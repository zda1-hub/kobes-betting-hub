...
- Checkout Worker `kobes-betting-hub-checkout-staging` is deployed at `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev`, version `4eb7f294-5558-406f-bbde-b27935c7da7c`. Its post-deploy health check returned `200`; its cron is disabled; the isolated Discord guild/role bindings are present; `STRIPE_GLOBAL_PAYOUTS_KEY` is intentionally absent.
...
- Stripe test products already present in the intended `Kobesbettinghub` test account are pinned in staging configuration: monthly product `prod_V8mOKsNfRQSQbk` / price `price_1U8UpxE6p9BmPii3GoBjAXII` ($32.99/month) and starter product `prod_V8mOyRBS66iOwo` / price `price_1U8UpLE6p9BmPii3ErYC4FPr` ($10 one-time). Secure replacement restricted key `Kobe Betting Hub Staging v2` is installed only as encrypted Cloudflare secret `STRIPE_SECRET_KEY`; the first key whose value appeared in accessibility output is unused and quarantined pending explicit revocation. Correct-account event destination `we_1UF61QE6p9BmPii30UGuNEMO` sends the seven membership/refund/dispute events to the staging Worker, and its signing secret is encrypted as `STRIPE_WEBHOOK_SECRET`. The test Customer Portal exposes invoice history and payment-method updates and cancels at the end of the billing period while collecting a reason. The unrelated `Kobe Referral Sandbox` account is out of scope.
- Discord staging application `Kobe Betting Hub Staging` (`1548556799513333770`) exists and its isolated callback `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev/discord/callback` is saved. Its replacement client secret, bot token, and independent OAuth-state secret are encrypted in Cloudflare staging. Test guild `Kobe Betting Hub Staging` (`1548568770123927562`) and paid-member role `KBH Staging Member` (`1548568981906919464`) are bound to the Worker. The bot's final `Manage Roles` authorization is prepared but not granted pending action-time owner confirmation. The extra cross-environment callback remains pending explicit deletion. No test checkout, charge, subscription, cancellation, role grant/removal, payout, or production write has occurred.
...
| 2 | Membership lifecycle | **Partial; isolated staging ready for bot authorization and lifecycle test** | Production membership controls remain untouched. Separate staging Supabase `ctqmksvfqrysomvckqrm`, static-site Worker `1ce02b72-1f87-460e-a6b5-1187408af831`, and Checkout Worker `4eb7f294-5558-406f-bbde-b27935c7da7c` are deployed; schema/RLS/empty-state and health checks passed; cron and payouts are disabled. Secure Stripe and Discord secrets are encrypted in Cloudflare; correct-account webhook `we_1UF61QE6p9BmPii30UGuNEMO` listens to seven events; Customer Portal cancellation is end-of-period; Discord app/guild/role IDs are isolated and bound. Full local suite passes 146/146. | After action-time owner confirmation, authorize the staging bot's `Manage Roles` grant and place its bot role above `KBH Staging Member`. Revoke the quarantined exposed Stripe key and remove the extra cross-environment Discord callback after explicit deletion confirmation. Then, with separate transaction-time confirmation, run one dedicated test identity through Checkout → link → portal → cancel → role removal and retain Stripe event IDs, Discord user/role evidence, Supabase rows, and timestamps. Separately run the referral lifecycle in Stripe test mode, including the seven-day eligibility transition and a test payout. |
...
- [ ] Finish the staging environment. Separate Supabase, encrypted server credential, Checkout Worker, static-site Worker, Stripe replacement credential/webhook/portal, Discord MFA credentials, test guild, and member role are deployed or configured and recorded. Remaining: authorize the bot's `Manage Roles` grant, put the bot role above the paid-member role, remove quarantined/cross-environment artifacts after explicit deletion confirmation, and complete the controlled lifecycle test.
