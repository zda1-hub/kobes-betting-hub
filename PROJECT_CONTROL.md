...
- The OpenAI reconciliation workflow is isolated in GitHub Actions and has the production Supabase connection. The original read-only organization Admin key was copied there during migration but its value appeared in browser automation accessibility output; treat that credential as exposed and rotate it before the first scheduled/manual GitHub run. Remove the old key from Render and revoke it after the replacement is installed.
...
| 5 | Provider billing reconciliation | **Partial / credential rotation required** | The first 48-hour import wrote 14 usage and three cost rows and attributed the incident traffic. A daily/manual least-privilege GitHub Actions workflow now exists with encrypted `DATABASE_URL` and `OPENAI_ADMIN_KEY` repository secrets and pinned expected project/key IDs. The original Admin-key value appeared in browser automation accessibility output during migration, so it must not be used for the acceptance run. | Create a replacement read-only finite-expiry Admin key, update only the GitHub secret, revoke the exposed key, remove it from Render, run the workflow manually, verify the correlated Supabase operation, then add X/Cloudflare billing exports where available and prove one zero-unclassified controlled window. |
...
| 2026-09-12 | Isolate OpenAI reconciliation in a daily/manual GitHub Actions workflow and install the production `DATABASE_URL` plus current read-only Admin key as encrypted repository secrets. During migration the Admin-key value appeared in browser automation accessibility output; it is now classified as exposed and must be replaced before the workflow acceptance run. | Zakai Martin | Workflow and database secret ready; credential rotation required |
