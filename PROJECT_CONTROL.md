...
| 2 | Membership lifecycle | **Partial; retention and terminal cancellation verified in staging** | Production membership controls remain untouched. Staging migration `009` is applied; exact source commit `e31e234966aae3b721bc75dc0c2853632a9d7110` is deployed as Checkout Worker `531ee711-60a1-4429-9147-6595ed13eb5a` and is healthy. Fresh clock `clock_1UFAPBE6p9BmPii34X93rbeb` proved role grant, paid `$8.25` invoice `in_1UFFj1E6p9BmPii37FBFrhWg`, return to `$32.99`, and no-refund period-end cancellation. Terminal event `evt_1UFFqTE6p9BmPii3UWYNvLwv` persisted Supabase `canceled`, `PROCESSED/ROLE_REMOVED`, and audited Discord DELETE `204`. Refund/dispute durable blocks and failed-payment handling pass automated tests; the Stripe staging destination listens to all eight events. | Prove the same member cannot accept the portal retention offer twice. Then run isolated external partial/full-refund, dispute creation/resolution, failed-payment/recovery, and audited restoration fixtures plus the referral lifecycle. |
...
| 9 | Legal/support operations | **Blocked on remaining owner/legal decisions** | Draft Terms, Privacy, support pages, and issue playbooks exist. The owner selected cancellation at period end, all sales final except legally/card-network-required or written exceptions, and a one-time 75%-off next-month retention offer before return to `$32.99/month`. Test coupon `VJxmdXFz` produced one `$8.25` invoice and returned to `$32.99`; per-customer repeat acceptance is not yet externally proven. The draft Terms remain unpublished and unapproved. | Supply the legal business identity/address, jurisdiction, support/privacy email, failed-payment grace rule, referral-program authority/termination language, tax reporting process, retention policy, backup operator, and legal approval; prove repeat-offer enforcement and publish timestamped policies. |
