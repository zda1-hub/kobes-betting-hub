...
- Checkout Worker `kobes-betting-hub-checkout-staging` is deployed at `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev`. Source deployment `105b81e4-f4d1-4cda-af7f-d41d6d51bde5` is active through final secret-change deployment `18b83c76-0f37-486d-aa09-bba6d73255d3`. Its post-rotation health check returned `200`; its cron is disabled; the isolated Discord guild/active-role bindings are present; `STRIPE_GLOBAL_PAYOUTS_KEY` is intentionally absent.
...
- Discord staging application `Kobe Betting Hub Staging` (`1548556799513333770`) exists with only the isolated callback `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev/discord/callback`; the extra cross-environment callback was removed. Its replacement client secret, bot token, and independent OAuth-state secret are encrypted in Cloudflare staging. Test guild `Kobe Betting Hub Staging` (`1548568770123927562`) contains the authorized bot with `Manage Roles`. Active paid-member role `KBH Staging Member Active` (`1548578573177061416`) is below the bot role and is the only member role bound to the Worker. Empty obsolete role `KBH Staging Member` (`1548568981906919464`) was verified by exact ID and deleted with owner confirmation; the final role list contains only the bot role and active member role. No test checkout, charge, subscription, cancellation, member role grant/removal, payout, or production write has occurred.
...
| 2 | Membership lifecycle | **Partial; isolated staging is ready for the controlled lifecycle test** | Production membership controls remain untouched. Separate staging Supabase `ctqmksvfqrysomvckqrm`, static-site Worker `1ce02b72-1f87-460e-a6b5-1187408af831`, and Checkout Worker source deployment `105b81e4-f4d1-4cda-af7f-d41d6d51bde5` / active secret deployment `18b83c76-0f37-486d-aa09-bba6d73255d3` are deployed; schema/RLS/empty-state, secret inventory, and health checks passed; cron and payouts are disabled. Correct-account webhook `we_1UF61QE6p9BmPii30UGuNEMO` listens to seven events; Customer Portal cancellation is end-of-period. Discord bot authorization, `Manage Roles`, isolated callback, guild, and active lower member role are verified. All exposed Stripe key values were expired immediately, the final replacement was installed without rendering, the obsolete empty role was deleted by exact ID, and the full local suite passes 146/146. | With separate transaction-time confirmation, run one dedicated test identity through Checkout → link → portal → cancel → role removal and retain Stripe event IDs, Discord user/role evidence, Supabase rows, and timestamps. Then exercise failed renewal/recovery, refund, and dispute state rules. Separately run the referral lifecycle in Stripe test mode, including the seven-day eligibility transition and a test payout. |
