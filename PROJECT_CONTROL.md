...
**Status:** Active production system preparing for a public-beta launch; customer-facing acceptance lifecycle remains open

**Last verified:** 2026-09-13 17:01 MST

**Production runtime change set:** repository/Render auto-deploy from `main` at `1a07657685951aa81298908ae86fc61c8ab6dab7`; Checkout Worker source `b187f252-1ff3-44fe-92d1-fa3b3c4a5b9d` with current secret-bearing version `d0a9c537-9480-4591-8948-22aaa598e9aa`; Publisher Worker `33bdf3f7-f0aa-4d8b-b133-9b0ec2212365`; public-site Worker `59485719-9cab-42eb-bf90-1074c21f0156`
...
- **Passed:** the Apps Script source is now version-controlled with four no-send/credential/trigger invariants. Three additional tests prove exact image-byte multipart publication, text-only fallback, and fail-closed credential behavior; the complete release suite passes `191/191`.
...
- **Active:** the private membership-operations alarm reads production Supabase in a read-only transaction every two hours and fails visibly on paid-but-unlinked memberships, unresolved reconciliation failures, failed webhooks, stuck webhooks, or a stale reconciliation. It emits counts/timestamps only, never customer, subscription, or Discord identifiers. Its first run passed before legacy import; its next run correctly exposed the two Stripe-only memberships and their access-sync failure.
- **Membership import corrected:** production reconciliation now discovers Stripe subscriptions rather than only rows already known to Supabase. It imported both observed live subscriptions and carries their Stripe-stored Discord IDs into the normalized customer rows. The remaining access failure is isolated to Discord returning `403`: server settings show the production Kobe Bot role has `Manage Roles` disabled.
...
- **Known exception:** two live `$32.99/month` subscriptions were observed under the same member email. Do not cancel either one until the intended subscription is identified and the owner confirms the exact action.
...
- `support@kobesbettinghub.com` is active and forwards to `themartinventures@gmail.com`. Published support hours and a response target are still undecided.
...
| 2 | Membership lifecycle | **Partial — live acceptance waiting** | The isolated test-clock lifecycle passed one-time 75%-off billing, return to `$32.99`, terminal cancellation, Supabase `canceled`, processed role removal, and Discord DELETE `204`. Stripe/Supabase/Discord production controls, portal routing, retry-safe Checkout idempotency, failed-payment suspension/recovery, dispute blocks, and checkout rate limiting are live in Checkout Worker `ff778d91-c34c-4b49-92c9-dfa02785fa37`. | Kobe must finish the intended live portal cancellation. Then reconcile Stripe/Supabase/Discord without changing another subscription and verify paid-through access. Monitor the later production terminal role removal during public beta. Identify the intended membership because two active live subscriptions were observed under the same email. |
...
| 8 | Membership reconciliation | **Complete for scheduled control; alert activation pending** | Checkout Worker `ff778d91-c34c-4b49-92c9-dfa02785fa37` runs Stripe → Supabase → Discord reconciliation daily at `15 16 * * *` (09:15 Arizona) and records outcomes/failures. A read-only GitHub Actions alarm now detects paid-without-access, reconciliation failures, failed/stuck webhooks, and stale reconciliation without printing identifiers. | Activate and run the alarm from the default branch, then prove one mismatch repair in a controlled test. |
| 9 | Legal/support operations | **Partial — customer pages live** | Effective Terms, Privacy, and support pages identify Kobe Irwin operating as Kobe's Betting Hub in California; disclose recurring billing, cancellation, no-refund exceptions, the 75%-off retention option, public beta, and dispute-based access suspension; `support@kobesbettinghub.com` forwards to the owner. | Choose published support hours/response target, retention periods, backup operator, age/jurisdiction controls, and legal/compliance reviewer. A street address is not required merely to prove the technical payment lifecycle, but applicable business/email-marketing laws may separately require one. |
...
3. What support hours and response target should be published during the public beta?
...
5. The operating identity, California jurisdiction, support email, cancellation/no-refund exceptions, retention offer, and payment-dispute suspension rules are approved. What age/jurisdiction restrictions, records-retention policy, and legal-review owner should be adopted?
