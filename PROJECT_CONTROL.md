...
**Last verified:** 2026-09-13 15:53 MST

**Production runtime change set:** repository/Render auto-deploy from `main` at `6989dd1b346508728dea43b3`; Checkout Worker `00cdf542-4592-4204-b0b2-ab5c6b2e536c`; Publisher Worker `33bdf3f7-f0aa-4d8b-b133-9b0ec2212365`; public-site Worker `59485719-9cab-42eb-bf90-1074c21f0156`
...
- **Passed:** the Apps Script source is now version-controlled with four no-send/credential/trigger invariants. Three additional tests prove exact image-byte multipart publication, text-only fallback, and fail-closed credential behavior; the complete release suite passes `183/183`.
...
| 7 | Free Pick mismatch | **Complete for text-only production; image transport tested locally** | A dedicated least-privilege credential is encrypted in Cloudflare and Render. Canonical pick `20260912-148-X` returned `201`, the current endpoint returned `200`, and the public page rendered it. Tests now prove exact approved PNG bytes are copied into one multipart publication, retrieval failure falls back once to text, and missing credentials fail closed. | Run one isolated staging image-backed publication and verify the retrieved media before using images in production. |
| 8 | Membership reconciliation | **Complete for scheduled control** | Checkout Worker `00cdf542-4592-4204-b0b2-ab5c6b2e536c` runs Stripe → Supabase → Discord reconciliation daily at `15 16 * * *` (09:15 Arizona) and records outcomes/failures. | Add an external alert destination for failed reconciliation and prove one mismatch repair in a controlled test. |
...
| 2026-09-13 | Bring the formerly Google-only daily email implementation under repository control and add four invariants covering zero-pick silence, no empty X queue call, no embedded credentials, and trigger-disabled recap polling. Add three image-backed Free Pick transport/fallback/fail-closed tests. | Zakai Martin | Complete; release suite `183/183` |
