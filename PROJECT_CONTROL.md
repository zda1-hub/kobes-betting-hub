...
**Last verified:** 2026-09-13 03:00 MST
...
- The dedicated test identity `832944927884312606` completed Discord OAuth, guild join, Stripe-to-Discord linking, role grant, authenticated Customer Portal entry, and owner-authorized cancellation at period end. Stripe test subscription `sub_1UF8Y4E6p9BmPii37J9VhpeS` remains `trialing` and is scheduled to cancel at `2026-09-20T08:14:02Z`; the role correctly remains active until that entitlement ends.
- The lifecycle exposed and fixed two additional staging defects: missing Supabase REST grants for the server credential and Stripe Basil's move of subscription period fields from the subscription root to subscription items. Migrations `007_service_role_rest_access` and `008_subscription_cancellation_fields` are applied only to staging. Final source deployment `30cea51e-5ef2-4d24-baae-f36565152726` passed health checks and persisted `cancel_at` plus the item-level period end.
- Final evidence includes processed webhooks `evt_1UF9bEE6p9BmPii3RxeS3DjL`, `evt_1UF9keE6p9BmPii3rJjT5igf`, `evt_1UF9kfE6p9BmPii3kga4qZ9I`, and `evt_1UF9paE6p9BmPii3E5s0nNcB`; Discord guild/role calls returned HTTP `204`; OAuth user/token calls, Stripe reads, and Checkout Session reads returned success. A best-effort OAuth token revocation returned HTTP `429`; this did not affect membership, and the short-lived token is not persisted.
- The remaining lifecycle observation is automatic role removal when Stripe emits the terminal cancellation event on September 20. Failed renewal/recovery, refund, and dispute scenarios also remain. No real payment, production write, or production Discord mutation occurred.
...
- The owner selected a one-time cancellation-retention offer: 75% off the next `$32.99` monthly invoice (approximately `$8.25`), then automatic return to `$32.99/month` until canceled. Customers must be able to decline the offer and continue cancellation without obstruction. Stripe Customer Portal supports a retention coupon, but this exact coupon is not yet configured or test-verified.
...
- Supabase project `Kobe Betting Hub Staging` (`ctqmksvfqrysomvckqrm`, East US / Ohio) is separate from production project `mpajyubbnnsdpgdizvht`. Repository migrations `001`–`008` are recorded; all 19 protected tables have RLS; `anon` and `authenticated` have zero table grants; and the server-only `service_role` has only the REST permissions required by the backend. Membership, webhook, membership-event, and API-call tables began with zero rows.
...
- Checkout Worker `kobes-betting-hub-checkout-staging` is deployed at `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev`. Final source deployment `30cea51e-5ef2-4d24-baae-f36565152726` includes the Stripe Basil compatibility fix; secret-only Discord client rotation deployment `5c878920-d8c8-449d-9922-ce4cfc849e77` installed the credential without exposing it. Health returned `200`; cron is disabled; the isolated Discord guild/active-role bindings are present; `STRIPE_GLOBAL_PAYOUTS_KEY` is intentionally absent.
...
- Stripe test products already present in the intended `Kobesbettinghub` test account are pinned in staging configuration: monthly product `prod_V8mOKsNfRQSQbk` / price `price_1U8UpxE6p9BmPii3GoBjAXII` ($32.99/month) and starter product `prod_V8mOyRBS66iOwo` / price `price_1U8UpLE6p9BmPii3ErYC4FPr` ($10 one-time). Every restricted-key value that surfaced in browser output was immediately rotated with expiration set to `now`; the final replacement was copied without rendering, installed only as encrypted Cloudflare staging secret `STRIPE_SECRET_KEY`, and the local clipboard was cleared. Correct-account event destination `we_1UF61QE6p9BmPii30UGuNEMO` sends the seven membership/refund/dispute events to the staging Worker, and its signing secret is encrypted as `STRIPE_WEBHOOK_SECRET`. The test Customer Portal exposes invoice history and payment-method updates and cancels at the end of the billing period while collecting a reason. The unrelated `Kobe Referral Sandbox` account is out of scope.
- Discord staging application `Kobe Betting Hub Staging` (`1548556799513333770`) has two intentional staging callbacks: the membership callback `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev/discord/callback` and the referral-sandbox callback. Its final client secret, bot token, and independent OAuth-state secret are encrypted in Cloudflare staging; no secret value is committed. Test guild `Kobe Betting Hub Staging` (`1548568770123927562`) contains the authorized bot with `Manage Roles`. Active paid-member role `KBH Staging Member Active` (`1548578573177061416`) is below the bot role and is the only member role bound to the Worker. Empty obsolete role `KBH Staging Member` (`1548568981906919464`) was verified by exact ID and deleted with owner confirmation. Test user `832944927884312606` received the active role, entered the Stripe portal through Discord identity authentication, and scheduled cancellation. Supabase now records `cancel_at=2026-09-20T08:14:02Z` and `current_period_end=2026-09-20T08:14:02Z`; final role removal is expected only when the entitlement ends. No real payment, referral payout from the membership account, or production write occurred.
...
Remaining controls after the production cutover: unmatched-account recovery, an external alert path for reconciliation/role failures, support tickets, required refund exceptions and chargeback handling, terminal role-removal observation, and an approved public policy. Daily Stripe-to-Supabase-to-Discord reconciliation is deployed.
...
| 2 | Membership lifecycle | **Partial; lifecycle verified through scheduled cancellation** | Production membership controls remain untouched. Separate staging Supabase `ctqmksvfqrysomvckqrm`, static-site Worker `1ce02b72-1f87-460e-a6b5-1187408af831`, and Checkout Worker `30cea51e-5ef2-4d24-baae-f36565152726` are healthy; cron and payouts are disabled. Dedicated test identity `832944927884312606` completed Checkout → Discord OAuth/link → guild join → role grant → authenticated Customer Portal → cancel at period end. Webhooks `evt_1UF9bEE6p9BmPii3RxeS3DjL`, `evt_1UF9keE6p9BmPii3rJjT5igf`, `evt_1UF9kfE6p9BmPii3kga4qZ9I`, and `evt_1UF9paE6p9BmPii3E5s0nNcB` were processed; Discord role calls returned `204`. Migrations `007`/`008` fixed service-role REST access and Stripe Basil period/cancellation persistence. Subscription `sub_1UF8Y4E6p9BmPii37J9VhpeS` is scheduled to end `2026-09-20T08:14:02Z`, with the role correctly retained until then. | Observe and retain the terminal Stripe cancellation webhook plus Discord role removal on September 20. Then exercise failed renewal/recovery, refund, and dispute state rules. Separately run the referral lifecycle in Stripe test mode, including the seven-day eligibility transition and a test payout. |
| 3 | Luna/X monitoring proof | **Complete locally and for the authorized extraction smoke test** | `X_TEST_CANDIDATE_LIMIT=1 node pipeline/test-real-x-pick.js --no-discord` checked one recent `@CappersUSA` post through `gpt-5.6-luna`; outcome was `SOURCE_EXTRACTED`. A new fully mocked non-public test now covers extraction → ESPN event/roster gate → identical private/public copy → canonical log → final ESPN grade/unit math → recap → X copy → Trends generation. The full local suite passes 160/160. | The next normal window must prove bounded production behavior in the durable ledger. After deliberate deployment, capture controlled real Discord, ESPN, Gmail, Trends, and X delivery evidence. Automatic grading remains limited to supported MLB/NFL/NCAAF markets; unsupported picks require `/grade-pick` so they do not hold the recap in `PENDING`. |
| 4 | Durable API trail | **Partial / deployment acceptance pending** | Supabase migrations `001`–`005` are live; explicit Node and Checkout Worker calls write endpoint/outcome/latency/hash/workflow metadata; eight production ESPN events were observed immediately after cutover. Publisher Worker `33bdf3f7-f0aa-4d8b-b133-9b0ec2212365` audits all outbound X calls. Commit `2b1fc85` additionally prepares authenticated Apps Script queue-call audit rows in append-only D1 and Discord SDK no-response failure rows in Supabase without retaining credentials, payloads, identifiers, or error text; the full suite passes 132/132. | Deploy the Worker and Render changes, update the authorized Apps Script project, capture one normal Apps Script D1 row and one controlled Discord no-response Supabase row, then reconcile zero unclassified provider traffic for a controlled window. |
...
| 7 | Free Pick mismatch | **Production text path complete; image path prepared locally** | A dedicated least-privilege credential is encrypted in Cloudflare and Render. Canonical pick `20260912-148-X` returned `201`, the current API and public page rendered it, and today's five-check production smoke passed. Local image work now carries an approved `/publish-pick` attachment into website/X publication, accepts only non-empty JPEG/PNG/WebP up to 5 MB, preserves exact bytes/type, prevents duplicate text-plus-image X posts, and reports partial delivery. The 160-test suite covers the combined path. | Deliberately deploy, then use one controlled approved image to verify the Discord card, website image/media bytes, exactly one X post, and audit rows. The monitored-source approval UI still cannot add an image; the usable image workflow is private `/preview-pick` followed by manual `/publish-pick` to `#daily-free-play`. |
...
| 9 | Legal/support operations | **Blocked on remaining owner/legal decisions** | Draft Terms, Privacy, support pages, and issue playbooks exist. The owner selected cancellation at period end, all sales final except legally/card-network-required or written exceptions, and a one-time 75%-off next-month retention offer before return to `$32.99/month`. The draft Terms record those choices but remain unpublished and unapproved. | Supply the legal business identity/address, jurisdiction, support/privacy email, failed-payment grace rule, referral-program authority/termination language, tax reporting process, retention policy, backup operator, and legal approval; then configure/test the Stripe retention coupon and publish timestamped policies. |
...
- [x] Complete the immediately observable test-identity lifecycle: Checkout, Discord link, role grant, portal authentication, and cancel at period end. The scheduled terminal role-removal observation remains due on 2026-09-20.
...
| 2026-09-13 | Add a non-public end-to-end pick fixture covering Luna-shaped extraction, ESPN validation, private/public copy equality, canonical logging, final grading/unit math, recap, X copy, and Trends generation. Harden image Free Pick publication to accept typed images up to 5 MB, preserve stored media, route manual approved attachments to the site/X path, and prevent duplicate image-plus-text X posts. | Zakai Martin | Local only; 160/160 tests pass, no deployment/post/email occurred, controlled external acceptance remains |
