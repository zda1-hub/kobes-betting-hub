...
**Last verified:** 2026-09-12 21:58 MST
...
- Supabase project `Kobe Betting Hub Staging` (`ctqmksvfqrysomvckqrm`, East US / Ohio) is separate from production project `mpajyubbnnsdpgdizvht`. All seven repository migrations are recorded; all 19 protected tables have RLS; `anon` and `authenticated` have zero table grants; membership, webhook, membership-event, and API-call tables began with zero rows.
...
- Checkout Worker `kobes-betting-hub-checkout-staging` is deployed at `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev`, version `105b81e4-f4d1-4cda-af7f-d41d6d51bde5`. Its post-deploy health check returned `200`; its cron is disabled; the isolated Discord guild/active-role bindings are present; `STRIPE_GLOBAL_PAYOUTS_KEY` is intentionally absent.
...
- Stripe test products already present in the intended `Kobesbettinghub` test account are pinned in staging configuration: monthly product `prod_V8mOKsNfRQSQbk` / price `price_1U8UpxE6p9BmPii3GoBjAXII` ($32.99/month) and starter product `prod_V8mOyRBS66iOwo` / price `price_1U8UpLE6p9BmPii3ErYC4FPr` ($10 one-time). The original exposed restricted key was expired and disappeared from the active-key list. Replacement `Kobe Betting Hub Staging v2` remains installed as encrypted Cloudflare secret `STRIPE_SECRET_KEY`, but its value appeared in a browser screenshot while verifying the first revocation; it must now be securely replaced and expired before a lifecycle test. Correct-account event destination `we_1UF61QE6p9BmPii30UGuNEMO` sends the seven membership/refund/dispute events to the staging Worker, and its signing secret is encrypted as `STRIPE_WEBHOOK_SECRET`. The test Customer Portal exposes invoice history and payment-method updates and cancels at the end of the billing period while collecting a reason. The unrelated `Kobe Referral Sandbox` account is out of scope.
- Discord staging application `Kobe Betting Hub Staging` (`1548556799513333770`) exists with only the isolated callback `https://kobes-betting-hub-checkout-staging.kobedirwin.workers.dev/discord/callback`; the extra cross-environment callback was removed. Its replacement client secret, bot token, and independent OAuth-state secret are encrypted in Cloudflare staging. Test guild `Kobe Betting Hub Staging` (`1548568770123927562`) contains the authorized bot with `Manage Roles`. Because Discord would not reorder the pre-existing role, active paid-member role `KBH Staging Member Active` (`1548578573177061416`) was created after bot installation and is below the bot role; the Worker is bound to it. Empty obsolete role `KBH Staging Member` (`1548568981906919464`) remains untouched pending explicit deletion authority. No test checkout, charge, subscription, cancellation, member role grant/removal, payout, or production write has occurred.
...
| 2 | Membership lifecycle | **Partial; isolated staging awaits final Stripe-key rotation and lifecycle test** | Production membership controls remain untouched. Separate staging Supabase `ctqmksvfqrysomvckqrm`, static-site Worker `1ce02b72-1f87-460e-a6b5-1187408af831`, and Checkout Worker `105b81e4-f4d1-4cda-af7f-d41d6d51bde5` are deployed; schema/RLS/empty-state and health checks passed; cron and payouts are disabled. Correct-account webhook `we_1UF61QE6p9BmPii30UGuNEMO` listens to seven events; Customer Portal cancellation is end-of-period. Discord bot authorization, `Manage Roles`, isolated callback, guild, and active lower member role are verified. Original exposed Stripe key was expired; the extra callback was removed. Full local suite passes 146/146. | Securely create/install a replacement for `Kobe Betting Hub Staging v2`, whose value surfaced during revocation verification, then expire v2. Optionally delete the empty obsolete Discord role after explicit confirmation. With separate transaction-time confirmation, run one dedicated test identity through Checkout → link → portal → cancel → role removal and retain Stripe event IDs, Discord user/role evidence, Supabase rows, and timestamps. Separately run the referral lifecycle in Stripe test mode, including the seven-day eligibility transition and a test payout. |
...
- [ ] Finish the staging environment. Separate Supabase, encrypted server credential, Checkout Worker, static-site Worker, Stripe webhook/portal, Discord MFA credentials, bot authorization, test guild, and assignable member role are deployed or configured and recorded. Remaining: rotate the newly exposed v2 Stripe key, optionally delete the empty obsolete Discord role, and complete the controlled lifecycle test.
