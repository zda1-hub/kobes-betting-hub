...
- **Still required before public-beta invitations:** read-only reconcile Kobe's live cancellation across Stripe, Supabase, and Discord and verify access remains through the paid-through date. Production role removal at the later entitlement end can be monitored as a public-beta acceptance item because the isolated terminal path has already passed. Do not publish a test pick publicly without explicit approval.
