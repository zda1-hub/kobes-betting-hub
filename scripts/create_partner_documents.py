...
def build_agreement():
    ...
    clause(d, 6, "Advertising standards and disclosures", "Partner will make truthful, supportable statements and clearly disclose the paid relationship close to each endorsement (for example, “Paid partnership with Kobe’s Betting Hub” or “Ad — I may earn a commission for paid memberships through this link”). Partner will use available platform disclosure tools in addition to, not instead of, a clear disclosure in the content. Arbitrage monitoring must be described as a beta feature that identifies potential, time-sensitive opportunities and requires members to verify current lines before wagering. Partner will not claim or imply guaranteed winnings, risk-free betting, certain profit, insider information, fixed games, or a specific win rate unless KBH has supplied current written substantiation and approved the exact claim.")
...
def build_kit():
    ...
    add_bullets(d, [
        ...
        "Describe arbitrage as monitoring for potential opportunities—not guaranteed profit—and state that sportsbook lines can move, limit, or void wagers.",
