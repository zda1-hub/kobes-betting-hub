...
test('production recap wiring keeps the button gate, private-channel preflight and optional-email independence', async () => {
  ...
  assert.match(source, /workflow\.engine\.decide\(\{ customId: interaction\.customId/);
  ...
  assert.match(command, /entry\.workflow\.engine\.prepare\(entry\.groups\)/);
  assert.doesNotMatch(command, /channel\.send|destinationFor\(/);
  assert.match(source, /recapApprovalWorkflows\.map\(\(workflow\) => workflow\.engine\.stop\(\)\)/);
});
