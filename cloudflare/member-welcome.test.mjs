...
function example(offer = 'trial_2_day') {
  ...
  const session = { id: 'cs_live_verified123', mode: 'subscription', status: 'complete', payment_status: long || offer === 'starter' ? 'paid' : 'no_payment_required',
    customer: 'cus_verified', subscription: 'sub_verified', currency: 'usd', amount_total: long ? amount : offer === 'starter' ? 1000 : 0,
    metadata: { offer }, customer_details: { email: 'member@example.com' } };
  const subscription = { id: 'sub_verified', customer: 'cus_verified', status: long ? 'active' : 'trialing', metadata: { offer },
    trial_end: long ? null : Math.floor(Date.now() / 1000) + 172800,
...
test('welcome covers both monthly intros and actual long-term prices', () => {
  for (const offer of ['starter', 'trial_2_day', 'referral_trial', 'six_month', 'annual']) {
