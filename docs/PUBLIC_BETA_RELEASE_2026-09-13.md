...
- The 14:05 Gmail message titled `Kobe's Betting Hub — Daily Picks — 2026-09-13` was not sent by the Render recap path: Render repeatedly logged that zero published picks caused no recap email. It came from the separately scheduled Gmail Apps Script. The repository's current Apps Script source already returns silently when no approved picks exist, so the installed Apps Script is stale or a second legacy trigger/function remains active. Replace the installed source and recreate its one daily trigger, or disable the legacy trigger; do not change it during the Discord approval incident.
