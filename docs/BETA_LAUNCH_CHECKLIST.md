...
- [ ] A current encrypted backup or point-in-time recovery method is verified for the production database. **Read-only check on 2026-09-13 confirmed the production Supabase project is on the Free Plan and its dashboard states that project backups are not included.** This remains a hard gate: create a securely stored encrypted logical backup and document a restore check, or upgrade and verify the selected recovery feature. Do not put credentials in the launch record.
