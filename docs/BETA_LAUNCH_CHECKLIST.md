...
- [x] A current encrypted logical backup is verified for the production database. GitHub run `34774738544` created an encrypted custom-format archive, decrypted a temporary copy, validated it with `pg_restore --list`, and produced a checksum-verified local copy. The key remains only in GitHub Secrets and macOS Keychain. A full disposable-database restore is still required before claiming a complete disaster-recovery drill; do not put credentials in the launch record.
