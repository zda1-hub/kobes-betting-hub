...
- [ ] Complete live content acceptance for one fresh private Trends card and one future legitimate approved-pick recap/social-package email. The fixture-only format path passes with no external delivery, both queue connection tests pass, and both five-minute workers are active.
- [x] Finish the backlog-safe recap and Trends triggers. With explicit owner approval, the existing Render recap credential was copied directly into Apps Script without entering the repository or audit text. The read-only recap connection and first scheduled recap worker run completed successfully. The first scheduled Trends run exposed a missing `Kobe Trends` Gmail label; the label was created and an immediate empty-label worker run completed with no external delivery. Activation timestamps continue to exclude the old recap rows and old mail.
