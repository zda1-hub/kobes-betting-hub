...
- Found that `cancellation_completed` analytics events were read by the dashboard but never written by the Stripe subscription webhook. A small idempotent webhook emission and explicit pre-Sep-23 historical-unavailable state are local pending release. It changes no cancellation or VIP-role lifecycle logic.
